...
# import — this line launches the simulator process.
...
# Locate Isaac's bundled robot assets; abort cleanly if missing.
...
# Full paths to the Kaya (3 omni-wheels) and Jetbot (2 wheels) models.
...
# Return world position + Euler angles. Isaac gives quaternion [w,x,y,z];
# scipy wants [x,y,z,w], hence the reorder. euler[2] is yaw (degrees).
def get_pose(robot):
...
# Straight-line XY distance between two points (ignores Z).
def euclid2d(a, b):
...
# Pretty-print the stats table; sd maps each metric to its 5 run-values.
...
# Measure Jetbot wheel radius + wheelbase from the model geometry
# (nothing hardcoded): wheelbase = distance between the left/right wheel
# prims; radius = half the wheel bounding-box extent across its spin axis.
...
# with wheel params measured from geometry.
...
# Print one EXP6 run: each robot's distance, yaw, and measured wheel params.
...
# One EXP6 run: build scene, drive Kaya + Jetbot straight for 300 steps,
# measure how far each travelled.
def run_exp6_single(seed):
    ...
    # Flat infinite floor to drive on.
    ...
    # MAIN LOOP: advance physics one frame at a time until 300 steps done.
    # Only counts a step while the sim is playing (responds to pause/stop).
...
# Run EXP6 five times (seeds 0-4), then print summary statistics.
def experiment_6():
    print("\n" + "#" * 90)
...
# COMMAND SETS (used by EXP D & E): each robot follows 10 [vx,vy,omega]
# slots forming a slightly different S-curve, so the formation flexes.
commands_robot1 = [
    (1.0,   0.0,   0.00000),
    (1.0,   0.05,  0.00010),
    (0.966, 0.198, 0.00030),
...
# EXP D: 5 Kaya robots lined up 2 m apart along Y.
...
# EXP E: 23 Kaya robots in three groups (I=inner, F=followers, L=line).
EXP_E_NAMES = [
    'I1','I2','I3','I4','I5',
...
# Starting distance between two EXP-E robots (varies per pair).
...
# Build a world and spawn one Kaya + controller at each named position.
...
# Drive the whole fleet 300 steps, snapshot positions + neighbor gaps
# every 30 steps, return final displacements and all snapshots.
def run_de_simulation(world, robots, controllers, cmd_map, robot_names,
    ...
    while simulation_app.is_running():
        ...
        if world.is_playing():
            ...
            # Current command slot: advances every steps_per_cmd, capped at the last.
...
# Print the per-30-step position table, then the neighbor-gap table.
...
# Print one run's final distances/yaws and how far each gap drifted.
...
# Shared driver for EXP D and E: 5 runs, then aggregate all stats.
...
# EXP D wrapper: 5 robots, fixed 2 m gaps.
def experiment_d():
    run_de_experiment(
...
# EXP E wrapper: 23 robots, per-pair initial gaps.
def experiment_e():
    run_de_experiment(
...
# Models an imperfect wireless link (used by EXP7): delay, jitter, packet
# loss, value corruption (noise), and random link outages.
class CommChannel:
    ...
    # Try to transmit one command; it may be lost, corrupted, delayed, or
    # blocked by an outage. A perfect link delivers instantly.
    def send(self, cmd):
        self.stats['sent'] += 1
        if self.perfect:
            self.last = tuple(cmd)
    ...
    # Advance one step and return whatever command has arrived by now; keep
    # the most recent; fall back to the last good command or the default.
...
# Bind a friction material to the ground (unless the case uses defaults).
...
# top surface at z=0. Each EXP7 case gets its own tile.
...
# Build the 60 cases as the full cross-product comm x friction x velocity.
E7_CASES = []
...
# One EXP7 run: build all 60 cases (120 robots) side by side, drive them
# 300 steps sending every command THROUGH its comm channel, then measure.
def run_exp7(seed):
    set_seed(seed)
    fresh_stage()
    ...
    # Drive loop: each step, commands go through send()/receive() on the link.
...
# Run EXP7 five times and aggregate per-case metrics into statistics.
def experiment_7():
    print("\n" + "#"*95)
...
# MAIN: run all four experiments in order, then close the simulator.
experiment_6()
experiment_d()
experiment_e()
experiment_7()
